        ...
        policy: [
          {
            ...
            "@id":
              "http://localhost:3000/static/references/rules/rule-access-1.json",
            title: {
              "@type": "xsd/string",
...
export const ConsentProvider = ({
  ...
  const onConsent = async () => {
    ...
    if (userInformation.email && !APP_REGEX.email.test(userInformation.email)) {
    ...
    const getPopulatedContracts = async (exchanges: Exchanges[]) => {
      const exchangePromises = exchanges?.map(async (exchange) => {
        const participantSelfDescription = async () => {
          ...
          const participant = await fetch(exchange?.participantSelfDescription);
      ...
      const removeDuplicates = (arr: AvailableExchangesPopulate[]) => {
        ...
        arr?.forEach((el) => {
          ...
          if (unique?.find((u) => u?.contracts?._id === el?.contracts?._id))
            return;
          else unique.push(el);
        });
    ...
    const populatedPrivacyNotice = async (notices: PrivacyNotice[]) => {
      try {
        const noticesPromises = notices?.map(async (notice) => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const recipientsPromises = notice?.recipients?.map(async (r: any) => {
            if (!r) return;
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const purposesPromises = notice?.purposes?.map(async (p: any) => {
        ...
        console.error(error);
        throw new Error("Failed to populate the current privacy notice");
      }
    };
