...
export const ConsentProvider = ({
    ...
    const onConsent = async () => {
        ...
        console.log(sessionCheckEndpoint)
    ...
    const consentLogin = async (email: string, password: string) => {
        //login the user on the consent
        const res = await fetch(`${consentEndpoint}/users/login`, {
            method: "POST",
            body: JSON.stringify({
                email,
                password
    ...
    useEffect(() => {
        console.log(privacyNoticeEndpoint)
        if (demo) return;
        let isMounted = true;

        const getPrivacyNotice = async () => {
            try {
                console.log('privacyNoticeEndpoint', privacyNoticeEndpoint)
        ...
        const getPopulatedContracts = async (exchanges: Exchanges[]) => {
            const exchangePromises = exchanges?.map(async (exchange) => {
                const participantSelfDescription = async () => {
                    const checkUrl =
                        exchange?.participantSelfDescription?.includes("http");//TODO
        ...
        const getAvailableExchanges = async () => {
            try {
                const response = await fetch(
                    `${consentEndpoint}/consents/exchanges/user?participantId=${participantId}`,
                    {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
    ...
    useEffect(() => {
        ...
        const currentPrivacyNoticeEndPoint: string =
            `${consentEndpoint}/consents/${user._id}/${availableExchanges.participant.base64SelfDescription}/${currentExchange.base64SelfDescription}/user`
        ...
        const getPrivacyNotices = async () => {
            ...
            try {
                ...
                const populate = await populatedPrivacyNotice(json);
                if (populate) {
        ...
        const getPrivacyNotice = async () => {
            ...
            try {
                const res = await fetch(
                    `${consentEndpoint}/consents/privacy-notices/${currentPrivacyNoticeSelected}/user`,
                    {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
