  ...
  const renderBody = () => {
      ...
      <>
        <div className={Styles.participantContainer}>
          <div className={Styles.participant}>
            <img
              src={
                privacyNotice?.dataProvider?.logo?.includes("https")
            ...
            <div className={Styles.controllerIdentify}>
              <span>{privacyNotice?.dataProvider?.legalName}</span>
              ...
              {privacyNotice?.data?.map((d, index) => (
                <React.Fragment key={d?._id + index}>
                  <div className={Styles.information}>
                    ...
                    {d?.b2cDescription && <span>{d?.b2cDescription}</span>}
                  </div>
                  {d?.softwareResources?.length > 0 && (
                    <div className={Styles.dataResources}>
                      {d?.softwareResources?.map((sr, i: number) => (
                        <div className={Styles.resourceItem} key={sr?._id + i}>
                  ...
                  {d?.dataResources?.length > 0 && (
                    <div className={Styles.dataResources}>
                      {d?.dataResources?.map((dr, i: number) => (
                        <div className={Styles.resourceItem} key={dr?._id + i}>
                          <span className={Styles.resourceItemName}>
                            {dr?.name}
                          </span>
        ...
        <div className={Styles.emailContainer}>
          ...
          <p>
            Please specify if there is another email associated with your
            account on {privacyNotice?.dataProvider?.legalName}. Otherwise,
            indicate that you do not have one.
          </p>
          <div className={Styles.emailInputContainer}>
            <input
              ...
              onChange={(e) =>
                updateUserInformation("email", e.currentTarget.value)
              }
            />
            <InputCheckbox
              name="checkbox-trigger"
              onChange={(e) =>
                updateUserInformation("trigger", e.currentTarget.checked)
              }
            >
            ...
            <div className={Styles.organisationContent}>
              {privacyNotice?.recipients?.map((r, i) => (
                <CardCollapse
                  ...
                  avatar={
                    r?.logo?.includes("https")
                      ? { src: r?.logo, alt: `Logo ${r?.legalName}` }
                      : {
                ...
                >
                  {privacyNotice?.purposes?.map((p, index) => (
                    <div key={p?._id + index}>
                      <p>
                        <b>{p?.name}</b>
                      </p>
                      ...
                      {p?.location && (
                        <p>
                          <b>You can find this data here:</b> {p?.location}
                        </p>
                      )}
                      <div style={{ marginBlock: 8 }}>
                        {p?.softwareResources?.map((sr) => (
                          <CardCollapseSoftwareResource softwareResource={sr} />
                        ))}
                      </div>
                    </div>
                  ))}
            ...
            <div className={Styles.yourDataSharing}>
              {privacyNotice?.data?.map((d, index) => (
                ...
                >
                  ...
                  {d?.location && (
                    <p>
                      <b>You can find this data here:</b> {d?.location}
                    </p>
                  )}
            ...
            <div className={Styles.legalInformation}>
              {ld.information.map((info, i) => (
                <React.Fragment key={info.title + i}>
                  {info.title && (
                    <h5 className={Styles.informationTitle}>{info.title}</h5>
                  )}
          ...
          {availableExchangesPopulate &&
            availableExchangesPopulate?.filter(
              ...
              <div className={Styles.containerItems}>
                ...
                {availableExchangesPopulate?.map((exchange, i: number) => {
                    ...
                    return (
                      <div
                        className={`${Styles.exchangeItem} ${
                          exchangeSelected === exchange?.contracts?._id
                            ? Styles.active
                            : ""
                        }`}
                        key={i}
                        onClick={() =>
                          handleExchangeSelected(exchange?.contracts?._id)
                        }
                      >
                        <p>
                          <b>{dataProviderName}</b>
                        </p>
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p>{exchange?.contracts?.ecosystem?.description}</p>
                      </div>
                      <div>{dataConsumerName}</div>
                    </div>
                  );
    ...
    const privacyNoticeBody = (
      <div className={Styles.privacyNoticeContainer}>
        {privacyNotices?.map((pn, index) => (
          <div
            className={`${Styles.item} ${
              pn?._id === privacyNoticeSelected ? Styles.active : ""
            }`}
            key={pn?._id + index}
        ...
        <ConsentModalFooter privacyNoticeSelected={privacyNoticeSelected} />
      </div>
    );
    const successBody = (
